...
def my_lstm_layer(input_reps, lstm_dim, input_lengths=None, scope_name=None, reuse=False, is_training=True,
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=reuse):
        ...
        #     lstm = tf.compat.v1.keras.layers.CuDNNLSTM(lstm_dim,go_backwards=True)
        #     # outputs, _ = lstm(inputs)
        #     print(lstm(inputs).)
        #     print(tf.transpose(lstm(inputs), [1, 0, 2]).shape)
        #     outputs = tf.transpose(outputs, [1, 0, 2])
        #     f_rep = outputs[:, :, 0:lstm_dim]
        #     b_rep = outputs[:, :, lstm_dim:2*lstm_dim]
        # else:
        context_lstm_cell_fw = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(lstm_dim)
        context_lstm_cell_bw = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(lstm_dim)
        if is_training:
            context_lstm_cell_fw = tf.compat.v1.nn.rnn_cell.DropoutWrapper(context_lstm_cell_fw, output_keep_prob=(1 - dropout_rate))
            context_lstm_cell_bw = tf.compat.v1.nn.rnn_cell.DropoutWrapper(context_lstm_cell_bw, output_keep_prob=(1 - dropout_rate))
        # context_lstm_cell_fw = tf.compat.v1.nn.rnn_cell.MultiRNNCell([context_lstm_cell_fw])
        # context_lstm_cell_bw = tf.compat.v1.nn.rnn_cell.MultiRNNCell([context_lstm_cell_bw])
        context_lstm_cell_fw = tf.keras.layers.StackedRNNCells([context_lstm_cell_fw])
        context_lstm_cell_bw = tf.keras.layers.StackedRNNCells([context_lstm_cell_bw])

        (f_rep, b_rep), _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(
...
def calcuate_attention(in_value_1, in_value_2, feature_dim1, feature_dim2, scope_name='att',
    ...
    with tf.compat.v1.variable_scope(scope_name):
        ...
        atten_w1 = tf.compat.v1.get_variable("atten_w1", [feature_dim1, att_dim], dtype=tf.float32)
        if feature_dim1 == feature_dim2: atten_w2 = atten_w1
        else: atten_w2 = tf.compat.v1.get_variable("atten_w2", [feature_dim2, att_dim], dtype=tf.float32)
        ...
        if att_type == 'additive':
            atten_b = tf.compat.v1.get_variable("atten_b", [att_dim], dtype=tf.float32)
            atten_v = tf.compat.v1.get_variable("atten_v", [1, att_dim], dtype=tf.float32)
            ...
            diagnoal_params = tf.compat.v1.get_variable("diagnoal_params", [1, 1, att_dim], dtype=tf.float32)
