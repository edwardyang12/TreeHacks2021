...
class SentenceMatchModelGraph(object):
    ...
    def create_placeholders(self):
        self.question_lengths = tf.compat.v1.placeholder(tf.int32, [None])
        self.passage_lengths = tf.compat.v1.placeholder(tf.int32, [None])
        self.truth = tf.compat.v1.placeholder(tf.int32, [None]) # [batch_size]
        self.in_question_words = tf.compat.v1.placeholder(tf.int32, [None, None]) # [batch_size, question_len]
        self.in_passage_words = tf.compat.v1.placeholder(tf.int32, [None, None]) # [batch_size, passage_len]

        if self.options.with_char:
            self.question_char_lengths = tf.compat.v1.placeholder(tf.int32, [None,None]) # [batch_size, question_len]
            self.passage_char_lengths = tf.compat.v1.placeholder(tf.int32, [None,None]) # [batch_size, passage_len]
            self.in_question_chars = tf.compat.v1.placeholder(tf.int32, [None, None, None]) # [batch_size, question_len, q_char_len]
            self.in_passage_chars = tf.compat.v1.placeholder(tf.int32, [None, None, None]) # [batch_size, passage_len, p_char_len]
    ...
    def create_model_graph(self, num_classes, word_vocab=None, char_vocab=None, is_training=True, global_step=None):
        ...
        if word_vocab is not None:
            ...
            with tf.device(cur_device):
                self.word_embedding = tf.compat.v1.get_variable("word_embedding", trainable=word_vec_trainable, 
        ...
        if options.with_char and char_vocab is not None:
            ...
            self.char_embedding = tf.compat.v1.get_variable("char_embedding", initializer=tf.constant(char_vocab.word_vecs), dtype=tf.float32)
        ...
        if options.with_highway:
            with tf.compat.v1.variable_scope("input_highway"):
        ...
        w_0 = tf.compat.v1.get_variable("w_0", [int(match_dim), int(match_dim/2)], dtype=tf.float32)
        b_0 = tf.compat.v1.get_variable("b_0", [int(match_dim/2)], dtype=tf.float32)
        w_1 = tf.compat.v1.get_variable("w_1", [int(match_dim/2), int(num_classes)],dtype=tf.float32)
        b_1 = tf.compat.v1.get_variable("b_1", [int(num_classes)],dtype=tf.float32)
        ...
        correct = tf.compat.v1.nn.in_top_k(logits, self.truth, 1)
        ...
        if self.options.optimize_type == 'adadelta':
            optimizer = tf.compat.v1.train.AdadeltaOptimizer(learning_rate=self.options.learning_rate)
        elif self.options.optimize_type == 'adam':
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.options.learning_rate)
