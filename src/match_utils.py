...
#     feat_dim = input_shape[2]
    ...
    with tf.compat.v1.variable_scope(scope or "highway_layer"):
        highway_w = tf.compat.v1.get_variable("highway_w", [output_size, output_size], dtype=tf.float32)
        highway_b = tf.compat.v1.get_variable("highway_b", [output_size], dtype=tf.float32)
        full_w = tf.compat.v1.get_variable("full_w", [output_size, output_size], dtype=tf.float32)
        full_b = tf.compat.v1.get_variable("full_b", [output_size], dtype=tf.float32)
        trans = tf.nn.tanh(tf.compat.v1.nn.xw_plus_b(in_val, full_w, full_b))
        gate = tf.nn.sigmoid(tf.compat.v1.nn.xw_plus_b(in_val, highway_w, highway_b))
...
def multi_perspective_match(feature_dim, repres1, repres2, is_training=True, dropout_rate=0.2,
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=reuse):
        ...
        if options.with_mp_cosine:
            mp_cosine_params = tf.compat.v1.get_variable("mp_cosine", shape=[options.cosine_MP_dim, feature_dim], dtype=tf.float32)
...
def match_passage_with_question(passage_reps, question_reps, passage_mask, question_mask, passage_lengths, question_lengths,
    ...
    dim = 0
    with tf.compat.v1.variable_scope(scope or "match_passage_with_question"):
        ...
        all_question_aware_representatins.append(tf.math.reduce_max(relevancy_matrix, axis=2,keepdims=True))
        all_question_aware_representatins.append(tf.math.reduce_mean(relevancy_matrix, axis=2,keepdims=True))
        dim += 2
        if with_full_match:
            if forward:
...
def bilateral_match_func(in_question_repres, in_passage_repres,
    ...
    with tf.compat.v1.variable_scope('context_MP_matching'):
        for i in range(options.context_layer_num): # support multiple context layer
            with tf.compat.v1.variable_scope('layer-{}'.format(i)):
                ...
                with tf.compat.v1.variable_scope('left_MP_matching'):
                ...
                with tf.compat.v1.variable_scope('right_MP_matching'):
    ...
    if options.with_match_highway:
        with tf.compat.v1.variable_scope("left_matching_highway"):
        ...
        with tf.compat.v1.variable_scope("right_matching_highway"):
    ...
    with tf.compat.v1.variable_scope('aggregation_layer'):
    ...
    if options.with_aggregation_highway:
        with tf.compat.v1.variable_scope("aggregation_highway"):
